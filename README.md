...
Telemt Docker image builder     -> docker-telemt/
```
...
`docker-telemt/` builds a local Telemt Docker image from official upstream release artifacts. It verifies upstream `.sha256`, uses a non-root distroless runtime by default, includes a healthcheck, and does not publish anything unless `PUSH=1` is explicitly set.
