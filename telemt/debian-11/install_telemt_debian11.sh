...
# Telemt installer for Debian 11 / bullseye servers.
...
# It is a drop-in replacement for install_telemt.sh on Debian 11:
# it uses the same state files and can continue a partially completed install.
...
ensure_https_frontdoor_available() {
  ...
  local sni_conf
  for sni_conf in /etc/nginx/modules-enabled/90-stream-sni.conf /etc/nginx/modules-enabled/60-stream-sni.conf; do
    if [[ -f "$sni_conf" ]] && grep -q 'telemt_backend' "$sni_conf"; then
      return 0
    fi
  done
...
if step_done backup; then
  ...
  backup_path /etc/nginx/modules-enabled/90-stream-sni.conf
...
if step_done nft_api_block && systemctl is-active --quiet local-hardening-nft 2>/dev/null; then
...
Type=oneshot
RemainAfterExit=yes
ExecStartPre=/bin/sh -c '/usr/sbin/nft list table inet local_hardening >/dev/null 2>&1 && /usr/sbin/nft delete table inet local_hardening || true'
ExecStart=/usr/sbin/nft -f /etc/nftables-local-hardening.conf
ExecReload=/bin/sh -c '/usr/sbin/nft list table inet local_hardening >/dev/null 2>&1 && /usr/sbin/nft delete table inet local_hardening || true; /usr/sbin/nft -f /etc/nftables-local-hardening.conf'
...
if step_done nginx_config && nginx_http_redirect_current; then
  ...
  if [[ -f /etc/nginx/modules-enabled/60-stream-sni.conf ]] && grep -q 'telemt_backend' /etc/nginx/modules-enabled/60-stream-sni.conf; then
    rm -f /etc/nginx/modules-enabled/60-stream-sni.conf
  fi
...
EOF
  ...
  write_file_root /etc/nginx/modules-enabled/90-stream-sni.conf 0644 root:root <<EOF
stream {
