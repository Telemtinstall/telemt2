...
Для осознанной переустановки с нуля:
  RESET_INSTALL_STATE=1 ./install_telemt.sh -lang ru
EOF
  else
    cat >&2 <<'EOF'
...
For a safe update:
  ./install_telemt.sh --update -lang en

For repair/diagnostics:
  ./install_telemt.sh --fix-nginx -lang en

For an intentional clean reinstall:
  RESET_INSTALL_STATE=1 ./install_telemt.sh -lang en
EOF
  fi
  exit 1
fi
