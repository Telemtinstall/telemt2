...
require_apt_systemd() {
  ...
  have apt-get || die "This installer supports Debian 13 with apt-get."
...
require_supported_os() {
  ...
  major="${version_id%%.*}"
  if [ "$os_id" != "debian" ] || [ "$major" != "13" ]; then
    if is_ru; then
      die "Неподдерживаемая ОС: $pretty. Этот установщик предназначен только для Debian 13. Для Ubuntu 24-26 используйте каталог telemt/ubuntu-24-26."
    fi
    die "Unsupported OS: $pretty. This installer is only for Debian 13. Use telemt/ubuntu-24-26 for Ubuntu 24-26."
  fi
}
