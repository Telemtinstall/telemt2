...
def create_prompt_text_for_server(server_title: str) -> str:
    return "\n".join(
        [
            ...
            "",
            "Напишите имя клиента одним сообщением.",
            "Можно использовать латиницу, цифры, точку, дефис и нижнее подчеркивание.",
            "Пример: <code>home_router</code>",
            "",
            "Чтобы отменить создание, нажмите «Назад».",
        ]
    )


def create_method_text_for_server(server_title: str) -> str:
    return "\n".join(
        [
            ...
            "• <b>Указать имя</b> — бот попросит ввести имя пользователя;",
            "• <b>Создать автоматически</b> — бот сам сгенерирует имя.",
        ]
    )
