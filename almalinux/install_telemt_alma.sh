...
# then installs nginx SNI routing + Telemt + firewalld hardening.
...
ensure_https_frontdoor_available() {
  ...
  if [[ -f /etc/nginx/stream-conf.d/telemt-sni.conf ]] && grep -q 'telemt_backend' /etc/nginx/stream-conf.d/telemt-sni.conf; then
    return 0
  fi
...
nginx_http_redirect_current() {
  [[ -f /etc/nginx/conf.d/"$PUBLIC_HOST".conf ]] &&
    grep -Fq 'return 301 https://$host$request_uri;' /etc/nginx/conf.d/"$PUBLIC_HOST".conf
}
...
if step_done packages; then
  ...
  dnf install -y dnf-plugins-core ca-certificates curl openssl iproute \
    firewalld nginx nginx-mod-stream certbot jq \
...
EOF
  ...
  if ! grep -Eq '^[[:space:]]*include[[:space:]]+/etc/nginx/stream-conf\.d/\*\.conf;' /etc/nginx/nginx.conf; then
    cp -a /etc/nginx/nginx.conf "$BACKUP_DIR/nginx.conf.before-telemt-stream"
    cat >> /etc/nginx/nginx.conf <<'EOF'

# Telemt stream SNI routing include.
