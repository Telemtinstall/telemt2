...
# WireGuard installer for a fresh Debian/Ubuntu server.
# Modes:
#   1. Plain WireGuard: UDP 51820 by default.
#   2. HTTPS mask site: nginx serves a real HTTPS site on TCP 443,
#      WireGuard listens on UDP 443. TCP and UDP do not conflict.
...
BACKUP_ROOT="/root/wg-install-backups"
...
prompt_config() {
  ...
  valid_port "$WG_PORT" || die "Invalid UDP port: $WG_PORT"
...
Install plan:
  endpoint:        ${PUBLIC_ENDPOINT}:${WG_PORT}/udp
  interface:       ${WG_IFACE}
  subnet:          ${WG_SUBNET}
...
install_packages() {
  ...
  packages=(ca-certificates curl wireguard-tools iproute2 iptables qrencode)
  if [[ "$ENABLE_HTTPS_MASK" == "1" ]]; then
    packages+=(nginx certbot)
  fi
  retry_command "apt-get update" apt-get update
  retry_command "apt-get install WireGuard tools" apt-get install -y "${packages[@]}"
}
...
write_mask_http_config() {
  local root_dir conf
  root_dir="$(mask_root)"
  conf="/etc/nginx/sites-available/wg-mask-${MASK_DOMAIN}.conf"
...
EOF
  ln -sf "$conf" "/etc/nginx/sites-enabled/wg-mask-${MASK_DOMAIN}.conf"
  nginx -t
...
acme_http01_preflight() {
  ...
  token="wg-$(openssl rand -hex 8 2>/dev/null || date +%s)"
  expected="wg-acme-ok-${token}"
...
write_mask_https_config() {
  ...
  if [[ "$ENABLE_NGINX_LOGS" == "1" ]]; then
    access_log="/var/log/nginx/wg-mask-${MASK_DOMAIN}.access.log"
    error_log="/var/log/nginx/wg-mask-${MASK_DOMAIN}.error.log warn"
  fi
...
main() {
  detect_os
  load_resume_config
  echo "WireGuard installer for Debian/Ubuntu."
  echo
  echo "Before running:"
  ...
  echo "  3. Make sure the selected WireGuard UDP port is reachable from the internet."
