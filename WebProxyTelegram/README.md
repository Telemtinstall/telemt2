...
- если указан личный токен [IPinfo](https://ipinfo.io/), аналитика добавляет плитки
  стран, городов и провайдеров/ASN; если тариф IPinfo не возвращает город,
  город и регион дополняются через `ipwho.is` (не более 900 новых IP в сутки,
  с локальным кэшированием); токен сохраняется только на сервере в
