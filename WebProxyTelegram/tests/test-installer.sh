...
[[ "$source_text" == *'https://ipinfo.io/account/token'* ]] || fail "IPinfo token page missing from prompt"
[[ "$source_text" == *'LoadCredential=ipinfo_token:'* ]] || fail "protected IPinfo credential missing"
[[ "$source_text" == *'IPINFO GEOLOCATION'* ]] || fail "IPinfo summary status missing"
[[ "$source_text" == *'systemctl restart webproxy-analytics'* ]] || fail "analytics upgrade restart missing"
...
[[ "$source_text" == *'token_admin_ready'* ]] || fail "token admin startup wait missing"
...
if [ -f "$PROJECT_DIR/analytics/collector.py" ]; then
  ...
  grep -Fq 'Нажмите: показать города' "$PROJECT_DIR/analytics/app.js" || fail "country drill-down missing"
