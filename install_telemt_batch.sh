...
INSTALLER_FILE="${INSTALLER_FILE:-./install_telemt.sh}"
INSTALLER_README_FILE="${INSTALLER_README_FILE:-./install_telemt_README.md}"
...
The main installer disables SSH password login, so the server should already
have /root/.ssh/authorized_keys. If it does not, install_telemt.sh will stop
...
run_remote_installer() {
  ...
  # Empty email keeps install_telemt.sh default: admin@domain.
  # shellcheck disable=SC2086
  ssh $opts "$target" 'chmod +x /root/install_telemt.sh /root/add_key.sh 2>/dev/null || true; bash /root/install_telemt.sh' <<EOF
$domain
$email_answer
$TARGET_SSH_PORT
