...
# Batch orchestrator for install_telemt_alma.sh.
...
The Alma installer disables SSH password login, so the server should already
have /root/.ssh/authorized_keys. If it does not, install_telemt_alma.sh will stop
...
run_remote_installer() {
  ...
  # Empty email keeps install_telemt_alma.sh default: admin@domain.
  # shellcheck disable=SC2086
  ssh $opts "$target" 'chmod +x /root/install_telemt_alma.sh /root/add_key.sh 2>/dev/null || true; bash /root/install_telemt_alma.sh' <<EOF
$domain
$email_answer
$TARGET_SSH_PORT
...
fetch_proxy_links() {
  ...
  # Read the link generated by install_telemt_alma.sh. Fall back to the local API
...
EOF
  ...
  if [[ "$SSH_USER" != "root" ]]; then
    die "install_telemt_alma.sh must run as root. Use SSH_USER=root."
  fi
