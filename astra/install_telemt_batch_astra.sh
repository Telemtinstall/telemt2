...
# Batch orchestrator for install_telemt_astra.sh.
...
# then copies and runs install_telemt_astra.sh on each Astra Linux target.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

INSTALLER_FILE="${INSTALLER_FILE:-$SCRIPT_DIR/install_telemt_astra.sh}"
...
preflight_local_machine() {
  ...
  echo "Note: install_telemt_batch_astra.sh is a local orchestrator for Astra Linux targets."
...
run_remote_installer() {
  ...
  # Empty email keeps install_telemt_astra.sh default: admin@domain.
  # shellcheck disable=SC2086
  ssh $opts "$target" 'chmod +x /root/install_telemt_astra.sh /root/add_key.sh 2>/dev/null || true; bash /root/install_telemt_astra.sh' <<EOF
$domain
$email_answer
$TARGET_SSH_PORT
...
fetch_proxy_links() {
  ...
  # Read the link generated by install_telemt_astra.sh. Fall back to the local API
...
main() {
  ...
  cat <<'EOF'
Telemt batch installer for Astra Linux targets.

This script runs on your admin machine, which can be macOS or any Linux.
It installs Telemt on remote Astra Linux servers by copying and executing
install_telemt_astra.sh on each target server.

EOF

  prompt_default SSH_USER "SSH user for installation" "$SSH_USER"
  if [[ "$SSH_USER" != "root" ]]; then
    die "install_telemt_astra.sh must run as root. Use SSH_USER=root."
  fi
