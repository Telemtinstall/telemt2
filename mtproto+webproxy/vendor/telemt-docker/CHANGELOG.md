...
- Created this isolated Docker installer candidate for the exact upstream
  Telemt `3.4.24` release. The stable `docker-telemt` directory remains
  unchanged.
- Fresh installs and default `--update` now build or pull the exact `3.4.24`
  release asset and verify its upstream `.sha256`; moving `latest` is still
  rejected.
- `--update` supports `3.4.23` and older Docker installs, rejects accidental
  downgrades when the running version is newer than the selected target, and
  verifies that the recreated container actually runs the requested version.
- The new `direct_relay_buffer_budget_max_bytes` and
  `me_writer_byte_budget_bytes` keys are intentionally not written to TOML.
  Telemt applies its safe built-in auto/default values, while the preserved
  strict config remains usable for rollback to `3.4.23`.

### Added

- Added post-start validation for the new Direct Relay, ME Writer, and Buffer
  Pool memory metrics exposed by Telemt `3.4.24` on the existing local metrics
  endpoint.
- Added automatic update rollback. After the pre-update backup is created, a
  failed image build, container start, version check, API check, metrics check,
  or active-probing check restores the previous config, compose, secret/link
  files, and attempts to restart the previous image.
