...
SCRIPT_VERSION="2026-07-17-telemt-3.4.24"
...
TELEMT_LATEST_COMPATIBLE_VERSION="${TELEMT_LATEST_COMPATIBLE_VERSION:-3.4.24}"
...
TELEMT_UPDATE_BACKUP_DIR=""
TELEMT_UPDATE_TRANSACTION_ACTIVE="0"
...
Опции:
  ...
  -update, --update
                  ...
                  При ошибке после бэкапа автоматически вернуть прежний
                  config/compose/image; более новую версию не понижать.
  -fix, --fix-nginx
...
Options:
  ...
  -update, --update
                  ...
                  and nginx configs. After a post-backup failure, restore the
                  previous config/compose/image; never downgrade a newer release.
  -fix, --fix-nginx
...
version_gt() {
  local current="$1" other="$2"
  [ "$current" != "$other" ] && version_ge "$current" "$other"
}
...
expected_telemt_version() {
  local version

  if version="$(version_from_image_ref "$TELEMT_IMAGE" 2>/dev/null)"; then
    printf '%s' "$version"
    return 0
  fi
  normalize_exact_telemt_version "$TELEMT_VERSION"
}
...
prevent_update_downgrade() {
  [ -n "$TELEMT_DETECTED_VERSION" ] || return 0

  if version_gt "$TELEMT_DETECTED_VERSION" "$TELEMT_UPDATE_TARGET_VERSION"; then
    if is_ru; then
      die "Обновление остановлено: установленный Telemt $TELEMT_DETECTED_VERSION новее целевой версии $TELEMT_UPDATE_TARGET_VERSION. Укажите совместимую версию не ниже установленной."
    else
      die "Update stopped: installed Telemt $TELEMT_DETECTED_VERSION is newer than target $TELEMT_UPDATE_TARGET_VERSION. Select a compatible version that is not older than the installed release."
    fi
  fi
}
...
verify_running_telemt_version() {
  local expected="$1" current="" attempt=1

  while [ "$attempt" -le 15 ]; do
    current="$(detect_running_telemt_version || true)"
    if [ -n "$current" ]; then
      if [ "$current" != "$expected" ]; then
        print_telemt_container_diagnostics
        if is_ru; then
          die "Запущена неправильная версия Telemt: ожидалась $expected, получена $current."
        else
          die "Wrong Telemt version is running: expected $expected, got $current."
        fi
      fi
      if is_ru; then
        say "Версия Telemt подтверждена: $current"
      else
        say "Telemt version verified: $current"
      fi
      return 0
    fi
    sleep 2
  ...
  print_telemt_container_diagnostics
  if is_ru; then
    die "Не удалось определить версию Telemt внутри запущенного контейнера."
  else
    die "Could not determine the Telemt version inside the running container."
  fi
}

verify_telemt_3424_metrics() {
  local expected="$1" metrics_file="/tmp/telemt-metrics.prom" metric

  version_ge "$expected" "3.4.24" || return 0
  if ! curl -fsS http://127.0.0.1:9090/metrics > "$metrics_file"; then
    if is_ru; then
      die "Endpoint метрик Telemt 3.4.24 не отвечает на 127.0.0.1:9090/metrics."
    else
      die "Telemt 3.4.24 metrics endpoint does not respond on 127.0.0.1:9090/metrics."
    fi
  fi

  for metric in \
    telemt_direct_relay_buffer_budget_bytes \
    telemt_me_writer_byte_budget_limit_bytes \
    telemt_buffer_pool_buffers_total
  do
    if ! grep -q "^${metric}" "$metrics_file"; then
      if is_ru; then
        die "В метриках Telemt нет обязательного показателя 3.4.24: $metric"
      else
        die "Required Telemt 3.4.24 metric is missing: $metric"
      fi
    fi
  done

  if is_ru; then
    say "Метрики управления памятью Telemt 3.4.24 подтверждены."
  else
    say "Telemt 3.4.24 memory-management metrics verified."
  fi
}
...
validate_install() {
  ...
  sleep 8
  expected_version="$(expected_telemt_version)" || die "Cannot determine the exact expected Telemt version."
  verify_running_telemt_version "$expected_version"
  ...
  verify_telemt_3424_metrics "$expected_version"
...
backup_update_state() {
  ...
  install -d -m 0700 "$backup_dir"
  TELEMT_UPDATE_BACKUP_DIR="$backup_dir"
...
restore_update_backup_file() {
  local destination="$1"
  local source="$TELEMT_UPDATE_BACKUP_DIR/$(basename "$destination")"
...
restore_update_core_state() {
  local restored_image attempt

  [ -n "$TELEMT_UPDATE_BACKUP_DIR" ] || return 1
  [ -f "$TELEMT_UPDATE_BACKUP_DIR/telemt.toml" ] || return 1
  [ -f "$TELEMT_UPDATE_BACKUP_DIR/docker-compose.yml" ] || return 1

  restore_update_backup_file "$INSTALL_DIR/telemt.toml"
  restore_update_backup_file "$INSTALL_DIR/docker-compose.yml"
  restore_update_backup_file "$SECRET_FILE"
  restore_update_backup_file "$SAVED_CONFIG"
  restore_update_backup_file /root/telemt-proxy-links.txt
  restore_update_backup_file /root/telemt-proxy-link.txt
  restore_update_backup_file /root/telemt-proxy-link-ip.txt

  restored_image="$(compose_image_from_file "$INSTALL_DIR/docker-compose.yml" || true)"
  [ -n "$restored_image" ] || return 1
  TELEMT_IMAGE="$restored_image"
  fix_runtime_permissions || true
  start_telemt || return 1
...
handle_update_exit() {
  ...
  [ "$TELEMT_UPDATE_TRANSACTION_ACTIVE" = "1" ] || return 0

  set +e
  if is_ru; then
    say "WARN: update завершился ошибкой около строки $line_number. Восстанавливаю прежний Docker Telemt из $TELEMT_UPDATE_BACKUP_DIR."
  else
    say "WARN: update failed near line $line_number. Restoring the previous Docker Telemt state from $TELEMT_UPDATE_BACKUP_DIR."
  fi

  if restore_update_core_state; then
    if is_ru; then
      say "Откат выполнен: прежний конфиг, compose, секреты и контейнер снова работают. Бэкап сохранен: $TELEMT_UPDATE_BACKUP_DIR"
    else
      say "Rollback completed: the previous config, compose, secrets, and container are running again. Backup kept at: $TELEMT_UPDATE_BACKUP_DIR"
    fi
  else
    if is_ru; then
      say "ОШИБКА: автоматический откат не завершился. Файлы прежней установки сохранены в $TELEMT_UPDATE_BACKUP_DIR"
    else
      say "ERROR: automatic rollback did not complete. Previous installation files remain in $TELEMT_UPDATE_BACKUP_DIR"
    fi
  fi
}
...
План обновления:
  ...
  synlimit:         $TELEMT_SYNLIMIT
  host nginx TLS:   $(host_nginx_tls_plan)
  память 3.4.24:    безопасные auto/default бюджеты Telemt; новые ключи в конфиг не добавляются для совместимости отката
...
Update plan:
  ...
  synlimit:         $TELEMT_SYNLIMIT
  host nginx TLS:   $(host_nginx_tls_plan)
  3.4.24 memory:    safe Telemt auto/default budgets; new config keys stay absent for rollback compatibility
...
EOF
  ...
  ensure_docker_available
  backup_update_state
  TELEMT_UPDATE_TRANSACTION_ACTIVE="1"
  trap 'handle_update_exit "$?" "$LINENO"' EXIT
  ensure_ubuntu_nginx_openssl35
  ensure_python3_for_idn
  refresh_docker_image_for_update
  apply_update_compatibility_patches
