...
TELEMT_VERSION="${TELEMT_VERSION:-3.4.24}"
...
Variables:
  ...
  TELEMT_VERSION     Exact upstream release tag. Default: 3.4.24
...
main() {
  ...
  if [ "${TELEMT_VERSION#refs/tags/}" = "latest" ] && [ "$ALLOW_TELEMT_LATEST" != "1" ]; then
    printf 'ERROR: TELEMT_VERSION=latest is disabled for reproducible builds. Use an exact release tag such as 3.4.24, or set ALLOW_TELEMT_LATEST=1 intentionally.\n' >&2
    exit 1
  fi
