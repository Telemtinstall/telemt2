# Telemt Docker 3.4.24 Candidate
...
> Локальная тестовая редакция: папка создана отдельно от стабильного
> `docker-telemt` и пока не опубликована. Для испытания 3.4.24 запускайте
> `install_docker-telemt.sh` непосредственно из этой папки; корневой
> `install.sh` и опубликованные GitHub URL до отдельной публикации ведут на
> стабильную редакцию.
...
tests/test-update-policy.sh Проверить target, сохранность config/secret и rollback.
```
...
if [ -d /root/telemt2/.git ]; then
  ...
  cd /root/telemt2
  git sparse-checkout set telemt/docker-telemt-3.4.24
fi
cd /root/telemt2/telemt/docker-telemt-3.4.24
...
mkdir -p /root/telemt2/telemt/docker-telemt-3.4.24
cd /root/telemt2/telemt/docker-telemt-3.4.24
wget -O Dockerfile https://raw.githubusercontent.com/Telemtinstall/telemt2/main/telemt/docker-telemt-3.4.24/Dockerfile
wget -O build.sh https://raw.githubusercontent.com/Telemtinstall/telemt2/main/telemt/docker-telemt-3.4.24/build.sh
wget -O install_docker-telemt.sh https://raw.githubusercontent.com/Telemtinstall/telemt2/main/telemt/docker-telemt-3.4.24/install_docker-telemt.sh
wget -O telemt-users.sh https://raw.githubusercontent.com/Telemtinstall/telemt2/main/telemt/docker-telemt-3.4.24/telemt-users.sh
chmod +x ./build.sh ./install_docker-telemt.sh ./telemt-users.sh
./install_docker-telemt.sh -lang ru
```
...
mkdir -p /root/telemt2/telemt/docker-telemt-3.4.24
cd /root/telemt2/telemt/docker-telemt-3.4.24
curl -fsSLo Dockerfile https://raw.githubusercontent.com/Telemtinstall/telemt2/main/telemt/docker-telemt-3.4.24/Dockerfile
curl -fsSLo build.sh https://raw.githubusercontent.com/Telemtinstall/telemt2/main/telemt/docker-telemt-3.4.24/build.sh
curl -fsSLo install_docker-telemt.sh https://raw.githubusercontent.com/Telemtinstall/telemt2/main/telemt/docker-telemt-3.4.24/install_docker-telemt.sh
curl -fsSLo telemt-users.sh https://raw.githubusercontent.com/Telemtinstall/telemt2/main/telemt/docker-telemt-3.4.24/telemt-users.sh
chmod +x ./build.sh ./install_docker-telemt.sh ./telemt-users.sh
./install_docker-telemt.sh -lang ru
```
...
```bash
docker run --rm --entrypoint /app/telemt telemt-local:3.4.24 --version
```
...
- Telemt `3.4.24`-совместимые настройки: `/run/telemt` runtime cache, `client_mss`, `client_mss_bulk`, `user_enabled`, upstream IPv4 policy, Synlimit V2-поля и явно выключенный `synlimit`.
...
cd /root/telemt2
git pull --ff-only
cd telemt/docker-telemt-3.4.24
chmod +x ./build.sh ./install_docker-telemt.sh ./telemt-users.sh
./install_docker-telemt.sh --update -lang ru
```

Обновить Docker Telemt `3.4.23` или более раннюю версию до проверенной совместимой версии `3.4.24`:
...
# если в плане указана текущая версия -> 3.4.24, запустить обновление
./install_docker-telemt.sh --update -lang ru
```
...
Режим `--update` сначала анализирует существующую установку: читает домен и пользователя из `telemt.toml`, image из compose, пытается определить текущую версию Telemt по контейнеру/image/tag и выводит список отсутствующих совместимых ключей. После подтверждения он делает бэкап `/opt/telemt-docker/telemt.toml`, `docker-compose.yml`, секретов, ссылок и nginx-конфигов, переключает compose image на точный tag проверенной совместимой версии (`telemt-local:3.4.24` по умолчанию), пересобирает/скачивает именно этот release tag, дополняет текущий `telemt.toml`/compose только отсутствующими безопасными ключами, пересоздает контейнер и заново выполняет проверку версии, API, новых memory-метрик и active probing. Ручные значения не перезаписываются; `TELEMT_VERSION=latest` в `--update` не используется и заменяется на pinned compatible version. Если обновление после создания бэкапа завершается ошибкой, установщик восстанавливает прежние config/compose/secret/links и пытается снова запустить старый image. Более новая уже установленная версия не понижается.
...
```bash
TELEMT_IMAGE=ghcr.io/Telemtinstall/telemt:3.4.24 ./install_docker-telemt.sh
```
...
| `Docker image Telemt` | Имя Docker image, который будет запускаться в compose. | `telemt-local:3.4.24`. | Оставить дефолт. Для registry указать точный tag, например `ghcr.io/Telemtinstall/telemt:3.4.24`. |
...
Если Docker hardening отключен, контейнер запускается проще: без `read_only`, `cap_drop`, `no-new-privileges`, `/tmp` tmpfs и без compose healthcheck. Runtime tmpfs `/run/telemt` остается включенным, потому что Telemt `3.4.24` пишет туда cache/state.
...
Для Telemt `3.4.24` конфиг также получает runtime-кеш в `/run/telemt`, `beobachten`-окно для JA3/JA4 диагностики, `client_mss = "tspu"` и `client_mss_bulk = "1400"` если MSS не отключен, `[access.user_enabled]`, per-upstream `ipv4/ipv6`, `mask_dynamic = false` для сохранения статической mask-схемы и `synlimit = false`. SYN limiter не включается автоматически в Docker/nginx-stream схеме, потому что для него нужен `CAP_NET_ADMIN`, а внешний `443/tcp` принимает nginx. Если оператор вручную включает `TELEMT_SYNLIMIT=nftables|iptables`, скрипт пишет новые Synlimit V2-поля `synlimit_ios_*` и `synlimit_hashlimit_*`.

Telemt 3.4.24 добавил `general.direct_relay_buffer_budget_max_bytes` и `general.me_writer_byte_budget_bytes`. Установщик намеренно не добавляет их в существующий или новый TOML: при отсутствии ключей Telemt применяет безопасный автоматический бюджет Direct Relay и штатный лимит ME Writer. Это сохраняет тот же конфиг пригодным для аварийного отката на 3.4.23, где строгая проверка незнакомых ключей могла бы остановить сервис. После запуска установщик проверяет `/metrics` и наличие `telemt_direct_relay_buffer_budget_bytes`, `telemt_me_writer_byte_budget_limit_bytes` и `telemt_buffer_pool_buffers_total`.
...
TELEMT_VERSION     Точный release tag. Default: 3.4.24
TELEMT_LATEST_COMPATIBLE_VERSION
                  Проверенная целевая версия для --update. Default: 3.4.24
...
tests/test-update-policy.sh Test target selection, config/secret preservation, and rollback.
```
...
- Telemt `3.4.24` compatibility settings: `/run/telemt` runtime cache, `client_mss`, `client_mss_bulk`, `user_enabled`, upstream IPv4 policy, Synlimit V2 fields, and explicit disabled `synlimit`.
...
Update Docker Telemt `3.4.23` or any earlier release to the checked compatible `3.4.24` release:
...
# if the plan shows the current version -> 3.4.24, run the update
./install_docker-telemt.sh --update -lang ru
```
...
The `--update` mode first analyzes the existing installation: it reads the domain and user from `telemt.toml`, the image from compose, tries to detect the current Telemt version from the container/image/tag, and prints missing compatible config keys. After confirmation it backs up `/opt/telemt-docker/telemt.toml`, `docker-compose.yml`, secrets, links, and nginx configs, switches compose to the exact checked compatible tag (`telemt-local:3.4.24` by default), rebuilds/pulls that exact release tag, extends the current `telemt.toml`/compose only with missing safe keys, recreates the container, and validates the running version, API, new memory metrics, and active probing. Manual values are not overwritten; `TELEMT_VERSION=latest` in `--update` is ignored and replaced with the pinned compatible version. If an error occurs after the backup, the installer restores the previous config/compose/secret/links and attempts to restart the old image. A newer installed Telemt version is never downgraded.
...
| `Telemt Docker image` | Docker image used by compose. | `telemt-local:3.4.24`. | Keep the default. For a registry image, use an exact tag such as `ghcr.io/Telemtinstall/telemt:3.4.24`. |
...
When Docker hardening is disabled, the container runs in a simpler mode without `read_only`, `cap_drop`, `no-new-privileges`, `/tmp` tmpfs, and compose healthcheck. The runtime tmpfs `/run/telemt` stays enabled because Telemt `3.4.24` writes cache/state there.
...
For Telemt `3.4.24`, the config also gets runtime cache under `/run/telemt`, a `beobachten` window for JA3/JA4 diagnostics, `client_mss = "tspu"` and `client_mss_bulk = "1400"` unless MSS is disabled, `[access.user_enabled]`, per-upstream `ipv4/ipv6`, `mask_dynamic = false` to preserve the static masking layout, and `synlimit = false`. SYN limiter is not enabled automatically in the Docker/nginx-stream layout because it needs `CAP_NET_ADMIN`, while public `443/tcp` is accepted by nginx. If the operator explicitly sets `TELEMT_SYNLIMIT=nftables|iptables`, the installer writes the newer Synlimit V2 `synlimit_ios_*` and `synlimit_hashlimit_*` fields.

Telemt 3.4.24 adds `general.direct_relay_buffer_budget_max_bytes` and `general.me_writer_byte_budget_bytes`. The installer intentionally leaves both keys absent in new and existing TOML files: Telemt then applies its safe automatic Direct Relay budget and default ME Writer limit. The same strict config therefore remains usable for an emergency rollback to 3.4.23, which could reject unknown keys. After startup, the installer checks `/metrics` for `telemt_direct_relay_buffer_budget_bytes`, `telemt_me_writer_byte_budget_limit_bytes`, and `telemt_buffer_pool_buffers_total`.
...
TELEMT_VERSION     Exact release tag. Default: 3.4.24
TELEMT_LATEST_COMPATIBLE_VERSION
                  Checked target version for --update. Default: 3.4.24
